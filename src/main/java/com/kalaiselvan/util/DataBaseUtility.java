...
public class DataBaseUtility {
    ...
    private static boolean isFailoverActive = false;
    ...
    /**
     * Updates the database connection details and recreates the connection pool.
     */
    public static void updateDataSource(String dbType, JsonObject newConfig) {
        System.out.println("Updating " + dbType + " database configuration...");
        closeDataSource();
        if (initializeDataSource(dbType)) {
            System.out.println("Data source reinitialized with new " + dbType + " configuration.");
        } else {
            System.err.println("Failed to connect with the new " + dbType + " configuration.");
        }
    }
